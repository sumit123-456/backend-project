...
Script to permanently delete specific employees from payroll system
 Deletes: Bob Johnson, Jane Smith, John Doe
"""
...
def delete_employee_data(employee):
    ...
    print(f"\n🗑️  DELETING ALL DATA FOR: {employee.first_name} {employee.last_name}")
    ...
    if total_records == 0:
        ...
        print("⚠️  This will permanently delete all related data!")
        
        # Confirm deletion
        confirm = input(f"\nAre you sure you want to permanently delete all data for {employee.first_name} {employee.last_name}? (yes/no): ")
        if confirm.lower() != 'yes':
            print(f"⏸️  Deletion cancelled for {employee.first_name} {employee.last_name}")
            return False
...
def main():
    ...
    print(f"\n⚠️  This will permanently delete ALL data for these {len(employees_to_delete)} employee(s)")
    print("including payroll, attendance, leave records, and all related data.")
    
    confirm_all = input(f"\nAre you sure you want to proceed with deleting all data for these employees? (yes/no): ")
    if confirm_all.lower() != 'yes':
        print("⏸️  Deletion cancelled by user.")
        return
