    ...
    try:
        ...
        if total_days2 <= 30:
            if leaves_used_this_month + total_days2 <= max_leaves_per_month:
                ...
                print(f"   WARNING: Would exceed monthly limit ({leaves_used_this_month + total_days2} > {max_leaves_per_month})")
        else:
            print(f"   FAIL: Exceeds 30-day application limit")
...
def test_leave_validation():
    ...
    for start, end in valid_ranges:
        ...
        print(f"   - {start} to {end}: {total_days} days - OK")
    
    print("\nTesting invalid date ranges:")
    for start, end in invalid_ranges:
        if start > end:
            print(f"   - {start} to {end}: End before start - Invalid")
        elif start < today:
            print(f"   - {start} to {end}: Past date - Invalid")
        else:
            total_days = (end - start).days + 1
            if total_days > 30:
                print(f"   - {start} to {end}: {total_days} days (exceeds 30-day limit) - Invalid")
